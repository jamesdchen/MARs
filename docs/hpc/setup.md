...
```bash
uv pip install 'claude-hpc[forecasting] @ git+https://github.com/jamesdchen/claude-hpc.git'
```

Without the extra, the predictor falls back to a diurnal moving-average
baseline — the calls still succeed.

**About upstream slash commands.** The claude-hpc repo only ships one slash
command (`/setup_hpc`); the often-referenced `/preflight`, `/submit-hpc`,
`/monitor-hpc`, `/aggregate-hpc`, `/campaign-hpc` are generated by that
installer into your user-global `~/.claude/commands/`. The MARs
experiment-runner does NOT depend on any of them — it invokes `hpc-agent`
directly via Bash. If you want them for interactive use outside MARs, run
`/setup_hpc` once from Claude Code after globally installing claude-hpc.
